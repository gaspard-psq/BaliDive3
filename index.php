<?php /* index.php */ ?>
<!doctype html>
<html lang="fr">
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width,initial-scale=1" />
  <title>Centre de plongée à Bali | Bali Dive Center</title>

  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link href="https://fonts.googleapis.com/css2?family=Quicksand:wght@400;600;700&display=swap" rel="stylesheet">

  <link rel="stylesheet" href="css/styles.css" />
  <link rel="stylesheet" href="css/index.css" />
</head>

<body>
<header class="entete">
  <input type="checkbox" id="nav-toggle" class="entete__toggleMenu" />

  <div class="entete__barre">
    <div class="entete__gauche">
      <label for="nav-toggle" class="entete__boutonMenu" aria-label="Ouvrir le menu">
        <span></span><span></span><span></span>
      </label>
    </div>

    <a class="entete__lienMarque" href="index.php">
      <img src="img/logo.png" alt="Logo Bali Dive Center" class="entete__logo" />
    </a>

    <div class="entete__droite">
      <a class="entete__lienPanier" href="panier.php">
        <span class="entete__iconePanier">🛒</span>
        <span class="entete__textePanier">Panier</span>
      </a>
    </div>
  </div>

  <label for="nav-toggle" class="entete__fondMenu"></label>

  <nav class="menuTiroir">
    <div class="menuTiroir__entete">
      <span class="menuTiroir__titre">Menu</span>
      <label for="nav-toggle" class="menuTiroir__fermer">✕</label>
    </div>

    <a class="menuTiroir__lien estActif" href="index.php">Accueil</a>
    <a class="menuTiroir__lien" href="offres.php">Offres</a>
    <a class="menuTiroir__lien" href="catalogue.php">Catalogue</a>
    <a class="menuTiroir__lien" href="contact.php">Contact</a>
  </nav>
</header>

<main class="contenuPrincipal">

  <!-- HERO -->
  <section class="banniereVideo">
    <video class="banniereVideo__video" autoplay muted loop playsinline>
      <source src="img/videoentre.mp4" type="video/mp4" />
    </video>
    <div class="banniereVideo__voile"></div>
    <div class="banniereVideo__contenu">
      <h2>Bienvenue sous les tropiques</h2>
      <p>Balidive vous accompagne pour une exploration sûre et confortable des fonds marins balinais.</p>
    </div>
  </section>

  <!-- PRESENTATION -->
  <section class="section section--pleineLargeur">
    <div class="conteneur deuxColonnes">
      <div class="deuxColonnes__texte">
        <h2>BaliDive</h2>
        <p>Partez à la découverte des fonds balinais avec Balidive. Entre récifs colorés, tombants impressionnants et rencontres inattendues, chaque plongée devient une petite aventure adaptée à votre niveau. Nous choisissons les sites selon la météo, votre expérience et vos envies du moment.</p>
        <p>Chaque sortie commence par un briefing simple et précis. Le matériel est prêt, l’équipe veille à votre sécurité, et vous pouvez vous concentrer entièrement sur l’exploration.</p>
        <p>Que vous rêviez de dénicher de petites créatures cachées, d’explorer des épaves mystérieuses ou de croiser les grands pélagiques, Balidive vous guide vers les spots qui feront vibrer votre curiosité et votre passion pour la plongée.</p>
      </div>
      <div class="deuxColonnes__media">
        <img src="img/plongee1.jpg" alt="Plongée à Bali" class="deuxColonnes__image" />
      </div>
    </div>
  </section>

  <!-- OFFRES -->
  <section class="offres">
    <div class="offres__voile"></div>

    <div class="conteneur offres__contenu">
      <div class="offres__intro">
        <h2>Découvrez nos offres</h2>
        <p>Choisissez l’expérience qui vous ressemble</p>
      </div>

      <div class="offres__grille">

        <article class="carteOffre">
          <h3>Offre essentiel</h3>
          <ul class="carteOffre__liste">
            <li>1 plongée de 1 à 2 heures (69€)</li>
            <li>Accessible aux débutants</li>
            <li>Bateau confortable (6–8 pers.)</li>
            <li>Découverte récifs & faune locale</li>
            <li>Eau fournie à bord</li>
          </ul>
          <a class="carteOffre__bouton" href="offres.php">En savoir plus</a>
        </article>

        <article class="carteOffre">
          <h3>Offre avancé</h3>
          <ul class="carteOffre__liste">
            <li>1 demi-journée – 4h (120$)</li>
            <li>Visite de 2 sites</li>
            <li>Conseils personnalisés</li>
            <li>Bateau 10–12 pers. avec toilettes</li>
            <li>Collation & boissons incluses</li>
          </ul>
          <a class="carteOffre__bouton" href="offres.php">En savoir plus</a>
        </article>

        <article class="carteOffre">
          <h3>Offre premium</h3>
          <ul class="carteOffre__liste">
            <li>1 journée complète – 8h (300$)</li>
            <li>Visite de 3 sites</li>
            <li>Accompagnement personnalisé</li>
            <li>Yacht haut de gamme</li>
            <li>Buffet & jacuzzi à bord</li>
          </ul>
          <a class="carteOffre__bouton" href="offres.php">En savoir plus</a>
        </article>

      </div>
    </div>
  </section>

</main>

<footer class="piedDePage">
  <div class="piedDePage__interieur">
    <div class="piedDePage__espace"></div>
    <div class="piedDePage__copyright">
      © <?php echo date('Y'); ?> BaliDive — Tous droits réservés
    </div>
    <nav class="piedDePage__navigation">
      <a class="piedDePage__lien" href="contact.php">Contact</a>
      <a class="piedDePage__lien" href="mentions-legales.php">Mentions légales</a>
    </nav>
  </div>
</footer>
</body>
</html>
